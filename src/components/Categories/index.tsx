...
import { Button } from '../Button'; // Importe o componente Button from '../Button';
  ...
  return (
    <section className='container flex gap-2.5 lg:grid lg:grid-cols-4 lg:gap-4 mb-10 overflow-x-auto scrollbar-hide snap-x snap-mandatory'>
      {categories.map((category, index) => (
        <div key={index} style={{backgroundImage: `url(${category.image})`}} className='h-125 bg-cover bg-center rounded-[20px] relative text-white shrink-0 w-[95%] md:w-1/2 lg:w-full flex justify-center items-center snap-center'>
          <div className='absolute inset-0 bg-black/30 rounded-[20px]'></div>
          <div className='relative'>
            <Button size='md' variant='secondary'>{category.name}</Button>
          </div>
        </div>
      ))}
    </section>
